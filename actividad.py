...
# DONE: Deben calcular el agente óptimo, o en su caso, el agente con mayor probabilidad de éxito.
optimal_agent_index = np.argmax(probability_matrix.mean(axis=1))
print(f"Optimal agent type: Agent {optimal_agent_index}")
...
# DONE: También analiza el desempeño de los agentes.
plt.figure(figsize=(10, 6))
sns.barplot(data=results_df1, x="k", y="clean_percentage")
plt.title("Performance of Agents: Clean Percentage by Number of Agents")
plt.xlabel("Number of Agents")
plt.ylabel("Clean Percentage")
plt.show()
